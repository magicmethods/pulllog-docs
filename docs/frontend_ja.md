...
フロントエンドリポジトリでは、重要なユーザーフローを検証するための **manifest-driven な Playwright E2E テストアーキテクチャ** も運用しています。
...
- 規定のデバイスマトリクス:
  ...
  - `ipad-pro-11`（タブレット）
  - `iphone-14`（スマートフォン）
- ケース定義は JSON マニフェストとして管理
- すべての E2E 実行で Markdown レポートを生成
- 成功時は PDF evidence としてアーカイブ可能
- レポートは共有テンプレートを利用し、日本語の PullLog 専用テンプレートセットにも対応

### レポーティング方針
- 1 つの case id は 1 つの論理シナリオを表します。
- 同じ case を複数プロジェクト／デバイスで実行した場合、生成されるケースレポートには **実行された全プロジェクトの結果** を集約して表示します。
- PDF evidence では、PC / Tablet / Smartphone の比較表を表示しつつ、各プロジェクトの詳細も保持できます。

> 本公開ドキュメントではアーキテクチャの概要のみを扱い、認証情報・内部エンドポイント・運用専用の詳細は掲載しません。
